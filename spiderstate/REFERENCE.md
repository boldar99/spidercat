...
*   **Stabilizer Finding (`spiderstate.verification`)**
    *   **Unified Mixed Fault Tracking**: The module no longer uses layer-by-layer `TrackedFaultSet` logic. Instead, it uses a unified tracking system that generates all possible mixed combinations of pre-verification faults and intra-layer single faults up to weight $t$. It precomputes an `active_errors` 3D boolean tensor `(num_faults, t, N)` that mathematically masks when each fault component occurs. This ensures precise tracking of inter-layer delay.
    *   `find_lookahead_verification_stabilizers`: Once column operations are chosen, this module finds a very efficient set of stabilizers to measure. It uses a beam search and greedy set-cover to operate in temporal layers (detecting 1, 2, ... $t$ faults progressively). Because `active_errors` is fully precomputed, lookahead coverage evaluation and target coverage validation are executed via instantaneous NumPy matrix multiplication (`(active_errors[:, l, :] @ H_l.T) % 2`). **Crucial Rule**: Basis cross-verification is required. To detect Z-faults, the algorithm must measure X-stabilizers, and to detect X-faults, it must measure Z-stabilizers.
...
*   **Benchmarks**: Below are some reliable tester configurations and expected performance metrics:
    *   **7_1_3**: Best FT result uses 11 CNOT gates + 1 measurement.
    *   **9_1_3**: Best FT result uses 11 CNOT gates + 1 measurement.
    *   **15_1_3**: Best FT result uses 27 CNOT gates + 2 measurement.
    *   **15_7_3**: Best FT result uses 25 CNOT gates + 1 measurement.
    *   **12_2_4**: Best FT result uses 36 CNOT gates + 5 measurements.
    *   **16_6_4**: Best FT result uses 57 CNOT gates + 8 measurements.
    *   **32_20_4**: Best FT result uses 132 CNOT gates + 21 measurements.
    *   **17_1_5**: Best FT result uses 57 CNOT gates + 9 measurements.
    *   **19_1_5**: Best FT result uses 82 CNOT gates + 16 measurements.
    *   **25_1_5**: Best FT result uses 73 CNOT gates + 13 measurements.
    *   **24_4_5**: Best FT result uses 141 CNOT gates + 26 measurements.
    ...
    *   **37_1_7**: Best FT result uses 388 CNOT gates + 112 measurements.
